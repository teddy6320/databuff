...
export default class FlameChartPlugin extends UIPlugin {
    ...
    initFirstSelect () {
        // console.log(this.initialClusterizedFlatTree, this.firstSelected)
        const { name, resource, service, _start, type, duration } = this.firstSelected || {}
        if (!this.firstSelected) {
            return
        }
        const marchSelected = this.initialClusterizedFlatTree.find(item => {
            const [__node] = item.nodes || []
            return __node && __node.name === name && __node.resource === resource && __node.service === service && __node._start === _start && __node.type === type && __node.duration === duration
        })
        if (marchSelected && marchSelected.nodes && marchSelected.nodes[0]) {
            this.selectedRegion = {
                type: 'node',
                data: marchSelected.nodes[0]
            }
        }
    }
    ...
    render() {
        ...
        if (this.selectedRegion && this.selectedRegion.type === 'node') {
            ...
            const { x, y, w } = this.calcRect(start, duration, level);
            // console.log('-----', start, duration, level)
            // 选中节点的颜色
            this.renderEngine.addStrokeToRenderQueue('#121317', x, y, w, this.renderEngine.blockHeight);
        }
