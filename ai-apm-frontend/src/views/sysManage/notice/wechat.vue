<template>
  <div class="setting-wrap flex-v" v-loading="isLoading">
    <el-form @submit.native.prevent ref="settingForm" :model="settingForm" class="setting-form flex-1" label-position="top" size="small">
      ...
      <el-form-item
        :label="$t('modules.views.dataReport.report.s_4c6e119c')"
        prop="robotEnable"
      ...
      >
        <el-switch
          ...
          :inactive-value="0"
        ></el-switch>
        <el-tooltip class="ml-10" :content="$t('modules.views.sysManage.notice.s_be3603a8')" placement="right" effect="light">
          <i class="db-icon-info describe"></i>
        </el-tooltip>
      </el-form-item>

      <el-form-item
        :label="$t('modules.views.sysManage.notice.s_0344a5c0')"
        prop="tenantEnable"
        :rules="{ required: true, message: $t('modules.views.sysManage.notice.s_2f5436ed'), trigger: 'change' }"
      >
        <el-switch
          ...
          :inactive-value="0"
        ></el-switch>
        <el-tooltip class="ml-10" :content="$t('modules.views.sysManage.notice.s_28bad9a8')" placement="right" effect="light">
          <i class="db-icon-info describe"></i>
        </el-tooltip>
      </el-form-item>
      ...
      <template v-if="settingForm.tenantEnable">
        <el-form-item label="CorpId" prop="corpid"
          :rules="{ required: true, message: $t('modules.views.sysManage.notice.s_8820fa9f'), trigger: 'blur' }">
          <el-input
            v-model="settingForm.corpid"
            maxlength="300"
            :placeholder="$t('modules.views.sysManage.notice.s_8820fa9f')"
            class="form-input" />
        </el-form-item>

        <el-form-item label="CorpSecret" prop="corpsecret"
          :rules="{ required: true, message: $t('modules.views.sysManage.notice.s_69621381'), trigger: 'blur' }">
          <el-input
            v-model="settingForm.corpsecret"
            maxlength="300"
            :placeholder="$t('modules.views.sysManage.notice.s_69621381')"
            class="form-input" />
        </el-form-item>

        <el-form-item label="AgentId" prop="wechatAgentId"
          :rules="{ required: true, message: $t('modules.views.sysManage.notice.s_af2f4ae9'), trigger: 'blur' }">
          <el-input
            v-model="settingForm.wechatAgentId"
            maxlength="300"
    ...
    <el-dialog :visible.sync="showTestModal"
      ...
      <el-form @submit.native.prevent ref="testForm" :model='testForm' :rules="testFormRules" label-width="80px" label-position="left" size="small">
        <el-form-item :label="$t('modules.views.sysManage.notice.s_92448a35')" prop='mobile'>
          <el-input v-model='testForm.mobile' :placeholder="$t('modules.views.sysManage.notice.s_47804f0f')"></el-input>
        </el-form-item>
...
export default class WechatSetting extends Vue {
    ...
    NoticeApi.getWeChatConfig()
      .then((rst: any) => {
        if (rst && rst.status === 200) {
          ...
          data.notifyType = data.notifyType || 'wechat'
  ...
  private setConfig (test?: boolean) {
    return new Promise((resolve, reject) => {
      this.$refs.settingForm.validate((valid: boolean) => {
        ...
        NoticeApi.setWeChatConfig(params)
          .then((rst: any) => {
            if (rst && rst.status === 200) {
  ...
  private testNotice () {
    this.$refs.testForm.validate((valid: boolean) => {
      ...
      this.postLoading = true
      NoticeApi.testWechatByPhone({ phone: this.testForm.mobile })
        .then((rst: any) => {
