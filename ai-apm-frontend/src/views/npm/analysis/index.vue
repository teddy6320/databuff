<template>
  <div class="analysis-wrapper">
    ...
    <el-drawer :visible.sync='showDetail' destroy-on-close size="742px">
      ...
      <traffic-detail v-if="showDetail" :detail="detail" :chartTypes="chartTypes" />
    </el-drawer>
  </div>
</template>
...
export default class Analysis extends Vue {
  ...
  private chartGroupKey = 'NPM_ChartGroup_Analysis'
  private defaultTypes: string[] = ['npm.volume_sent', 'npm.tcp_retransmit.pct', 'npm.rtt']
  ...
  private async getPerformanceMetrics () {
    const { result, error } = await toAsyncWait(NpmApi.getPerformanceMetrics());
    if (!error) {
      this.chartTypes = Object.values(result.data || {}).map((t: any) => ({
        label: t.desc,
        value: t.identifier,
      }))
    }
