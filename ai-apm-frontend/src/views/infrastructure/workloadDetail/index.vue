  ...
  <div class="db-detail-wrapper">
    <div class="db-detail" :class="`db-detail-${activeName}`" v-loading="isLoading">
      <div class="detail-title">
        <span class="db-icon-workload detail-icon mr-10"></span>
        <span class="detail-name">{{ getQueryParams.workloadName }}
          <i @click.stop="copyHandle(getQueryParams.workloadName || '-')" class="copy-icon db-icon-copy"></i>
        </span>
      </div>
...
export default class WorkloadDetail extends Vue {
  public $refs!: {
    overview: Overview
    baseinfo: Baseinfo
    metric: Metric
  ...
  get getQueryParams () {
    return {
      ...
      workloadId: this.workloadId || this.detail.uid || '',
      workloadName: this.workloadName || this.detail.name || '',
      workloadKind: this.detail.workloadKind || '',
    };
  }
  ...
  private created () {
    const { kid = '', kn = '' } = this.$route.query;
    this.workloadId = decodeURIComponent(kid as string)
    this.workloadName = decodeURIComponent(kn as string)
  ...
  private async getDetail () {
    const { fromTime, toTime, interval, workloadId } = this.getQueryParams
    const params: any = {
      resourceUid: workloadId,
      fromTime,
      toTime,
      interval,
      offset: 0,
    ...
    this.isLoading = true;
    const { result, error } = await toAsyncWait(KubernetesApi.getWorkloadList(params));
    this.isLoading = false;
    const data = ((result || {}).data || [])[0]
    if (!error && data) {
      this.detail = {
        ...data,
        workloadKind: WorkloadTypeMapping[data.type] || data.type || '',
        podCount: +data.replicasDesired || 0,
        cpuCapacity: (+data.cpuCapacity || 0) / 1000,
      ...
      this.detail = {}
      this.$confirm(i18n.t('modules.views.infrastructure.workloadDetail.s_799644e7') as string, i18n.t('common.hint') as string, {
