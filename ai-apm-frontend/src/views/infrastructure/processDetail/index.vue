  ...
  <div class="db-detail-wrapper">
    <div class="db-detail" :class="`db-detail-${activeName}`" v-loading="isLoading">
      <div class="detail-title">
        <span
          :class="detail._processState === 0 ? 'bg-green' : 'bg-grey'"
          class="detail-status">{{ detail._processState | ProcessStateFilter }}</span>
        <span class="detail-name">{{ processName || '-' }}
          <i @click.stop="copyHandle(processName || '-')" class="copy-icon db-icon-copy"></i>
        </span>
      </div>
...
export default class ProcessDetail extends Vue {
  ...
  private activeName: 'metric' | 'baseinfo' = 'metric'

  private created () {
    const { processName = '', hostName = '' } = this.$route.query;
    this.processName = decodeURIComponent(processName as string)
    this.hostName = decodeURIComponent(hostName as string)
  ...
  private async durationChangeHandle () {
    ...
    if (!this.processName || !this.hostName) {
      return;
    }
  ...
  private async getDetail () {
    ...
    this.isLoading = true;
    const { result, error } = await toAsyncWait(InfraApi.getProcessDetail(params));
    ...
    if (!error && data?.spuid) {
      ...
      this.detail = {
        ...data,
        _processState: ProcessOriginalStateFilter(data.processState),
        containerName: container.containerName || data.containerName || '',
        imageVersion: container.imageVersion || '',
        imageName: container.imageName || '',
        shortImageName: container.shortImageName || '',
      }
    } else {
      this.detail = {}
      this.$confirm(i18n.t('modules.views.infrastructure.processDetail.s_2088f540') as string, i18n.t('common.hint') as string, {
