...
export default class DetailMetric extends Vue {
  ...
  private async getChartData () {
    const { clusterId, nodeName, fromTime, toTime, interval } = this.queryParams
    const params = {
      query: {
        types: [],
        metric: '',
        ...
        from: [
          ...
          { left: 'host', operator: '=', right: nodeName, connector: 'AND' },
        ],
      },
    ...
    this.getChartSource(cpuData.metrics, params, cpuData, this.detail.cpuCapacity || 0)
    this.getChartSource(memoryData.metrics, params, memoryData, this.detail.memCapacity || 0)
  }
