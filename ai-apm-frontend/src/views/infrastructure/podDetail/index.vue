  ...
  <div class="db-detail-wrapper">
    <div class="db-detail" :class="`db-detail-${activeName}`" v-loading="isLoading">
      <div class="detail-title">
        <span class="db-icon-pod2 detail-icon mr-10"></span>
        <span class="detail-name">{{ getQueryParams.podName }}
          <i @click.stop="copyHandle(getQueryParams.podName || '-')" class="copy-icon db-icon-copy"></i>
        </span>
      </div>
      <div class="detail-info">
        ...
        <div class="info-item">Node:<span class="ml-8">{{ detail.nodeName || '-' }}</span></div>
        <div class="info-item">Workload:<span class="ml-8">{{ detail.wlName || '-' }}</span></div>
      </div>
...
export default class PodDetail extends Vue {
  public $refs!: {
    overview: Overview
    baseinfo: Baseinfo
    metric: Metric
  ...
  get getQueryParams () {
    return {
      ...
      podId: this.podId || this.detail.uid || '',
      podName: this.podName || this.detail.name || '',
      namespaceName: this.detail.namespace || '',
    };
  }
  ...
  private created () {
    const { kid = '', kn = '', cid = '', pn = '' } = this.$route.query;
    // pn podName 和cid一起用于区分开看板下钻来的
    if (cid && pn) {
      this.clusterId = decodeURIComponent(cid as string);
      this.podName = decodeURIComponent(pn as string);
    } else {
      this.podId = decodeURIComponent(kid as string)
      this.podName = decodeURIComponent(kn as string)
    }
  ...
  private async getDetail () {
    const { fromTime, toTime, interval, clusterId, podName } = this.getQueryParams
    const params: any = {
      fromTime,
      toTime,
      interval,
    ...
    if (this.podId) {
      params.resourceUid = this.podId
    } else if (clusterId) {
      params.clusterId = clusterId
      params.podName = podName
    }
    this.isLoading = true;
    const { result, error } = await toAsyncWait(KubernetesApi.getPodList(params));
    ...
    if (!error && data) {
      this.detail = {
        ...
        memCapacity: +data.memCapacity || 0,
      ...
      this.detail = {}
      this.$confirm(i18n.t('modules.views.infrastructure.podDetail.s_d897c9e9') as string, i18n.t('common.hint') as string, {
