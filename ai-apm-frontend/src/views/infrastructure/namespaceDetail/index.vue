  ...
  <div class="db-detail-wrapper">
    <div class="db-detail" :class="`db-detail-${activeName}`" v-loading="isLoading">
      <div class="detail-title">
        <span class="db-icon-namespace detail-icon mr-10"></span>
        <span class="detail-name">{{ getQueryParams.namespaceName }}
          <i @click.stop="copyHandle(getQueryParams.namespaceName || '-')" class="copy-icon db-icon-copy"></i>
        </span>
      </div>
...
export default class NamespaceDetail extends Vue {
  public $refs!: {
    overview: Overview
    baseinfo: Baseinfo
    metric: Metric
  ...
  get getQueryParams () {
    return {
      ...
      namespaceName: this.namespaceName || this.detail.name || '',
      namespaceId: this.namespaceId || this.detail.uid || '',
    };
  }
  ...
  private created () {
    const { kn = '', kid = '', cid = '' } = this.$route.query;
    this.namespaceName = decodeURIComponent(kn as string)
    // kn和cid一起用于区分开看板下钻来的
    if (cid && kn) {
      this.clusterId = decodeURIComponent(cid as string);
    } else {
      this.namespaceId = decodeURIComponent(kid as string);
    }
  ...
  private async getDetail () {
    const { fromTime, toTime, interval, clusterId, namespaceName } = this.getQueryParams
    const params: any = {
      fromTime,
      toTime,
      interval,
    ...
    if (this.namespaceId) {
      params.resourceUid = this.namespaceId
    } else if (clusterId) {
      params.clusterId = clusterId
      params.resourceName = namespaceName
    }
    this.isLoading = true;
    const { result, error } = await toAsyncWait(KubernetesApi.getNamespaceList(params));
    ...
    if (!error && data) {
      ...
      this.detail = {}
      this.$confirm(i18n.t('modules.views.infrastructure.namespaceDetail.s_e481d1cd') as string, i18n.t('common.hint') as string, {
