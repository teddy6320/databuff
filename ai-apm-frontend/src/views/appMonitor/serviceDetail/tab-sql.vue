    ...
    <div class="sql-wrapper flex-v">
      ...
      <div class="list-wrapper">
        <db-table
          ...
          tableKey='APM_SERVICE_DETAIL_SQL'
          ref='listTable'>
          <template slot='total' slot-scope="{ total }">
            <div class="flex-h-jc w-100p">
              <div class="describe">{{ $t('modules.views.alarmCenter.alarm.s_9ad9830c', { value0: new Intl.NumberFormat().format(total) }) }}</div>
              <el-button v-if='hasServiceCfgModule' @click="viewSetting"
                size='small' type="primary" icon="el-icon-plus" class="setting-resource-btn">{{ $t('modules.views.appMonitor.serviceAnalysis.s_7c57a563') }}</el-button>
            </div>
          </template>
...
export default class TabSql extends Vue {
  ...
  /** Database detail keeps serviceId as the DB target; web service detail scopes by caller. */
  get dbQueryScope () {
    const scope: any = {
      isSlow: 1,
      isIn: 1,
    ...
    if (this.current?.service_type === 'db' || this.$route.path === '/appMonitor/database/detail') {
      scope.dbTarget = 1
    }
    return scope
  }
