  ...
  <div
    ...
    <div class="type-list">
      <span
        ...
        :key="item.value"
        @click="toggleMetricTypeHandle(item.value)"
        :class="['type-item', { active: currMetricType === item.value }]"
      >{{ item.labelKey ? $t(item.labelKey) : item.label }}</span>
    </div>
...
const PoolMetricTypes = { type1: i18n.t('modules.views.appMonitor.dbConnPool.s_6b522b81') as string, type2: i18n.t('modules.views.appMonitor.dbConnPool.s_1ab064f1') as string, type3: i18n.t('modules.views.appMonitor.serviceDetail.s_46dc5c2e') as string }
const PoolNameTag = 'threadPoolName'

type MetricType = 'thread' | 'queue' | 'task'
...
export default class ThreadPool extends Vue {
  ...
  private metricTypes = [
    { label: i18n.t('modules.views.appMonitor.threadPool.s_4f66c26c') as string, labelKey: 'modules.views.appMonitor.threadPool.s_4f66c26c', value: 'thread' },
    { label: i18n.t('modules.views.appMonitor.threadPool.s_032eb71a') as string, labelKey: 'modules.views.appMonitor.threadPool.s_032eb71a', value: 'queue' },
    { label: i18n.t('modules.views.appMonitor.threadPool.s_937202d6') as string, labelKey: 'modules.views.appMonitor.threadPool.s_937202d6', value: 'task' },
  ]

  private currMetricType: MetricType = 'thread'

  private allMetricList: string[] = []
  private metricInfoMapping: any = {}

  get metricList () {
    return this.allMetricList.filter(t => this.metricInfoMapping[t]._type4 === this.currMetricType)
  }
  ...
  private async created() {
    ...
    if (this.metricTypes.find(t => t.value === metricType)) {
  ...
  private async getMetricList () {
    ...
    if (!error) {
      ...
      Object.values(data).forEach((item: any) => {
        const _metric = item.identifier.toLocaleLowerCase()
        let _type4 = 'thread'
        if (_metric.indexOf('queue') > -1) {
          _type4 = 'queue'
        } else if (_metric.indexOf('task') > -1) {
          _type4 = 'task'
        }
        item._type4 = _type4
      });
      this.metricInfoMapping = data
      this.allMetricList = orderBy(Object.keys(data), [t => t.toLocaleLowerCase()], ['asc']);
      this.allMetricList.forEach(metric => {
